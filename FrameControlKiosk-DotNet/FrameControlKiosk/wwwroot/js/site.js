...
function textKeyboard_Funcs() {
    ...
    $('#keyboard_text').on('click', 'button', function (e) {
        ...
        if ($focusedInput.is('input[type="text"]') || $focusedInput.is('input[type="search"]') || $focusedInput.is('input[type = "password"]') || $focusedInput.is('textarea.form-control')) {
        ...
        descText = descText.trimStart();
        $('#descriptionText').val(descText);
    ...
    $(document).on('focus', 'input[type="text"],input[type="password"],textarea.form-control', function (e) {
    ...
    $(document).on('click', function (e) {
        if (!$(e.target).is('input[type="text"],input[type="password"],textarea.form-control, #keyboard_text, #keyboard_text *')) {
            $('#keyboard_text').addClass('d-none').removeClass('shift-active');
            $lastFocusedInput = null;
        }
    });
...
$(document).on('touchstart touchmove', 'input[type="text"],input[type="password"],input[type="number"],textarea.form-control', function (e) {
...
function updateTime() {
    $.ajax({
        url: '/get-time', // Sunucunuzdaki URL
        type: 'GET',
        success: function (response) {
            $('#currentTime').text(response); // Sunucudan gelen zaman bilgisini güncelle
            $('#report_datetime').text(response); // Sunucudan gelen zaman bilgisini güncelle
            currentDate = response;

        },
        error: function () {
            $('#currentTime').text('Zaman bilgisi alınamadı.');
        }
    });

    $.ajax({
        url: '/api/command/get-usbInfo',
        type: 'GET',
        success: function (response) {
            $('.system_message').html("");

            $.each(response, function (i, item) {
                // Disk tanımlayıcısını ve fazladan boşlukları kaldır
                var cleanItem = item.split(' ').filter(function (part) {
                    return part.trim() !== '';
                }).slice(1).join(' '); // İlk elemanı atla
                //console.log(cleanItem);
                $('.system_message').append(cleanItem + '<br>');
            });
        },
        error: function () {
            console.log("usbInfo ERR");
        }
    });
...
function checkPassword() {
    ...
    var correctPassword = "4463Zbm95U";

    /*
    if (password === correctPassword) {
        window.location.href = '/station/DownloadJson'; // Uygun URL'yi kullanın
    } else {
        $.toast({
    ...
    */
    if (password === correctPassword) {
        ...
        // Tarihi 'dd/mm/yyyy' formatından 'mm/dd/yyyy' formatına çeviririz
        console.log(currentDate);
        // İlk olarak saat bilgisini kaldırıyoruz.
...
$('#datepicker').datepicker({
    ...
    autoclose: true,
    weekStart: 1, // Haftanın ilk gününü Pazartesi olarak ayarla
    language: 'tr',
    startDate: '01/01/2023',
}).on('changeDate', function (e) {
    // Tarih değiştiğinde çalışacak kodlar
    selectedReportDate = e.date; // Seçilen tarihi alır
    console.log(selectedReportDate); // Konsola seçilen tarihi yazdırır

    getReports(selectedReportDate);
    // İsteğe bağlı olarak, seçilen tarihi bir formatla string olarak almak için
    // var formattedDate = $(this).datepicker('getFormattedDate');
    // console.log(formattedDate);
});

function getReports(date) {
    $.ajax({
        url: '/station/getreport/' + date,
        type: 'GET',
        success: function (response) {
            //dataTable burada doldurulacak. datatable ın id si 'reportmainTable'
            console.clear();
            var table = $('#reportmainTable tbody');
            table.empty(); // Clear existing table rows

            response.forEach(function (report) {
                var row = $('<tr tabindex="0" class="report_title" onclick="focusRow(this);getReportDetail(' + report.id + ');"> ');
                row.append($('<td>').html('<div class="shadow-box">' + report.reportDate + '</div>'));
                row.append($('<td>').html('<div class="shadow-box">' + report.startingReportTime + '</div>'));
                row.append($('<td>').html('<div class="shadow-box">' + report.endingReportTime + '</div>'));
                row.append($('<td>').html('<div class="shadow-box">' + report.doneCount + '</div>'));
                row.append($('<td>').html('<div class="shadow-box">' + report.nok + '</div>'));
                if (report.done) {
                    row.append($('<td>').html(`<i class="fa-solid fa-check text-success"></i>`));
                } else {
                    row.append($('<td>').html(`<i class="fa-solid fa-x text-danger"></i>`));
                }
                table.append(row);
            });
...
function downloadReports() {
    window.location.href = '/station/download-report-json/' + selectedReportDate;
    openFileExplorer();
}
function openFileExplorer() {
    $.ajax({
        url: '/api/command/open-file-explorer',
        type: 'POST',
        success: function (response) {
...
function downloadReports() {
    ...
    $.ajax({
        url: '/station/download-report-json',
        type: 'post',
        data: JSON.stringify({ message: selectedReportDate }),
        contentType: "application/json; charset=utf-8",
        dataType: "blob", // Burayı blob olarak değiştirin
        success: function (response) {
            var blob = new Blob([response], { type: 'application/json' }); // Blob'u doğru şekilde oluştur
            var url = window.URL.createObjectURL(blob);
            var a = document.createElement('a');
            a.href = url;
            a.download = 'rapor_' + selectedReportDate.replace(/[/\\?%*:|"<>]/g, '') + '.json';
...
function getReportDetail(id) {
    setTimeout(function () {
        ...
        console.log("reportmainid: " + id);
        console.log(id);
        $.ajax({
            //url: form.attr('action'),
            url: '/station/getreport-details',
            type: 'post',
    ...
    }, 500); // Timeout kullanarak odağın hemen geri verilmesini sağla

}


// Satıra tıklandığında veya odaklandığında 'focused' sınıfını ekleyen fonksiyon
function focusRow(row) {
    // Önceki odaklanmış satırın sınıfını kaldır
    $('.report_title.focused').removeClass('focused');

    // Yeni tıklanan/odaklanan satıra 'focused' sınıfını ekle
    $(row).addClass('focused');
}

// Satırların click ve focus olaylarını ayarla
$('.report_title').on('click focus', function () {
    focusRow(this);
});